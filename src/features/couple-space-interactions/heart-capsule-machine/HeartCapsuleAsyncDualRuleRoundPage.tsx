...
import { generateAsyncDualRuleReply } from './generateAsyncDualRuleReply';
...
export function HeartCapsuleAsyncDualRuleRoundPage({
  user,
  partner,
  coupleSpace,
  chatHistory,
  ...
  const [ruleGuessDraft, setRuleGuessDraft] = React.useState('');
  const [submittedRuleGuess, setSubmittedRuleGuess] = React.useState<string | null>(draw.ruleGuess ?? null);
  const [error, setError] = React.useState<string | null>(null);
  const hasCompletedRound = Boolean(draw.roundCompletedAt);
  const hasRequestedOpeningRef = React.useRef(false);
  const hasFinishedInteraction = Boolean(submitted ?? draw.userAnswer) && Boolean(replyText || draw.resultReply);
  ...
  React.useEffect(() => {
    ...
    void generateAsyncDualRuleReply({
      activeConfig,
      user,
      partner,
      coupleSpace,
  ...
  const handleSubmit = React.useCallback(async () => {
    ...
    try {
      const reply = await generateAsyncDualRuleReply({
        activeConfig,
        user,
        partner,
        coupleSpace,
  ...
  const handleRevealRule = React.useCallback(() => {
    const guess = ruleGuessDraft.trim();
    if (!guess) return;

    const completedAt = Date.now();
    setSubmittedRuleGuess(guess);
    ...
    <div className="no-scrollbar h-full overflow-y-auto space-y-5 px-4 pb-[calc(var(--app-safe-area-bottom-ui,0px)+16px)] pt-2">
      ...
      <div className="rounded-[34px] border border-white/80 bg-white/82 p-5 shadow-[0_22px_60px_rgba(255,192,212,0.22)] backdrop-blur-md">
        ...
        <div className="mt-5 rounded-[28px] border border-rose-100/80 bg-gradient-to-b from-rose-50/90 to-white/95 p-4">
          <div className="text-xs font-medium tracking-[0.2em] text-rose-300">
            你先知道的是
          </div>
          <div className="mt-2 text-lg font-bold text-zinc-800">
            {isPartnerDraw ? capsule.partnerVisibleTask : capsule.userTask}
          </div>
          <div className="mt-3 text-sm leading-6 text-zinc-500">{capsule.roundGoal}</div>
          <div className="mt-4 rounded-[24px] border border-rose-100 bg-white/85 px-4 py-4 text-sm leading-7 text-zinc-600">
            {draw.selectedQuestion || capsule.summary}
          </div>
        </div>
        ...
        {!submitted && !hasFinishedInteraction ? (
          ...
          <div className="mt-5 space-y-4">
            {isPartnerDraw && (
              <div className="rounded-[26px] border border-rose-100/80 bg-white/88 px-4 py-4 shadow-sm">
                <div className="text-xs font-medium tracking-[0.18em] text-rose-300">他刚刚先递出来的是</div>
            ...
            {hasFinishedInteraction && !hasCompletedRound && (
              <div className="space-y-3 rounded-[26px] border border-rose-100/80 bg-white/88 px-4 py-4 shadow-sm">
                <textarea
                  value={ruleGuessDraft}
                  onChange={(event) => setRuleGuessDraft(event.target.value)}
                  rows={3}
                  placeholder={RULE_GUESS_PLACEHOLDER}
                  className="w-full resize-none rounded-[20px] border border-white/90 bg-white/85 px-4 py-3 text-[15px] leading-7 text-zinc-700 shadow-sm outline-none placeholder:text-zinc-400 focus:border-rose-200"
                />
                <button
                  type="button"
                  disabled={!ruleGuessDraft.trim()}
                  onClick={handleRevealRule}
                  className="w-full rounded-full bg-[linear-gradient(90deg,#ff9fc0_0%,#ffb6cd_50%,#ffc7d9_100%)] px-4 py-3 text-sm font-bold text-white shadow-[0_12px_24px_rgba(255,170,197,0.32)] disabled:opacity-50"
                >
                  猜猜他的规则是什么
                </button>
              ...
              <>
                <div className="rounded-[26px] border border-rose-100/80 bg-white/88 px-4 py-4 shadow-sm">
                  <p className="whitespace-pre-wrap text-[15px] leading-7 text-zinc-700">
                    {submittedRuleGuess ?? draw.ruleGuess}
                  </p>
                </div>

                <div className="rounded-[22px] border border-rose-100 bg-white/85 px-4 py-4 text-sm leading-7 text-zinc-500">
                  {capsule.partnerHiddenTask}
                </div>
              </>
            )}
