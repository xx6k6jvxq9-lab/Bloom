...
export function createSnapshotMemoryRecord(input: {
  ...
  return {
    ...
    sourceSessionType: input.sourceSessionType ?? ((input.sourceScene === 'group_chat' || input.sourceScene === 'group_offline') ? 'group' : 'direct'),
...
export function createLibraryNoteMemoryRecord(input: {
  ...
  return {
    ...
    kind: 'note',
    sourceScene,
    sourceSessionType: input.sourceSessionType ?? ((sourceScene === 'group_chat' || sourceScene === 'group_offline') ? 'group' : 'direct'),
    sourceSessionId: input.sourceSessionId ?? input.characterId,
    sourceEventIds: [],
    characterIds: [input.characterId],
    visibility: defaults.visibility,
