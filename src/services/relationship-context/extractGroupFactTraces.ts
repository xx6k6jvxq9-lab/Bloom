import type { ChatMessage } from '../../types';
import type { FactTraceWriteCandidate } from './buildFactTraceWriteRules';

type ExtractGroupFactTracesInput = {
  groupId: string;
  messages: ChatMessage[];
  memberIds: string[];
};

function getMessageText(message: ChatMessage): string {
  return (message.text || '').replace(/^[^:：]+[:：]\s*/, '').trim();
}

function inferFactType(text: string): FactTraceWriteCandidate['factType'] | null {
  if (/想吃|爱吃|喜欢吃|不吃|讨厌吃|最爱/.test(text)) return 'preference';
  if (/想去|打算|准备|计划|待会|等下|明天要|周末去/.test(text)) return 'plan';
  if (/最近|今天|刚下班|加班|生病|发烧|头疼|累|忙|困/.test(text)) return 'status';
  if (/一起|刚刚|上次|那次|刚看完|刚去了|刚玩了|一起去/.test(text)) return 'experience';
  if (/是我姐|是我哥|我爸|我妈|前任|同事|室友|发小|青梅竹马/.test(text)) return 'background';
  return null;
}

function inferSubjectType(
  text: string,
  senderCharacterId?: string,
): FactTraceWriteCandidate['subjectType'] {
  if (/我们|群里|大家/.test(text)) return 'group';
  if (/我|本人|最近我/.test(text) && senderCharacterId) return 'character';
  return 'group';
}

function inferSubjectId(
  text: string,
  groupId: string,
  senderCharacterId?: string,
): string {
  if (/我们|群里|大家/.test(text) || !senderCharacterId) {
    return groupId;
  }

  if (/我|本人|最近我/.test(text)) {
    return senderCharacterId;
  }

  return groupId;
}

function inferStability(factType: NonNullable<ReturnType<typeof inferFactType>>, text: string) {
  if (factType === 'background') return 'stable' as const;
  if (factType === 'experience') return 'situational' as const;
  if (/最近|今天|刚刚|待会|等下|明天/.test(text)) return 'temporary' as const;
  return 'situational' as const;
}

export function extractGroupFactTraces(
  input: ExtractGroupFactTracesInput,
): FactTraceWriteCandidate[] {
  return input.messages
    .filter((message) => message.role === 'model' && !!message.senderCharacterId && !message.isSystem)
    .slice(-12)
    .flatMap((message) => {
      const text = getMessageText(message);
      if (!text) return [];

      const factType = inferFactType(text);
      if (!factType) return [];

      return [{
        sourceScene: 'group_chat' as const,
        factType,
        subjectType: inferSubjectType(text, message.senderCharacterId),
        subjectId: inferSubjectId(text, input.groupId, message.senderCharacterId),
        relatedCharacterIds: message.senderCharacterId ? [message.senderCharacterId] : [],
        groupId: input.groupId,
        visibility: 'group_public' as const,
        stability: inferStability(factType, text),
        confidence: 'explicit' as const,
        summary: text,
        timestamp: message.timestamp,
        isExplicit: true,
        isPublic: true,
      }];
    });
}
