...
function buildLoveLetterReplyTaskSection(
  ...
  const lines = [
    ...
    '### 写法要求',
    '1. 只从 char 当下怎么被这封情书碰到出发来回应；可以抓住这封信里最触动 char 的一句话、一个措辞或一个点来回，但不要写成逐句阅读理解，也不要只剩抽象态度词堆叠。',
    '2. 可以有关系感，但关系感只能来自 char 现在的回应，不能靠任何故事、例子、场景、动作、物件来制造。',
    '3. recentContext 和最近聊天只能作为理解来源，不能复述，不能拼接，不能扩写成具体经历。',
    '4. 所有表达先过人设，再过阶段。宁可更少、更硬、更克制，也不要说出不符合这个 char 的泛用甜话或泛用傲娇话。',
    '5. 可以只回应最触动 char 的那一小部分，不需要接满，也不要为了完整而一股脑说完。',
    ...
    '4. 禁止八股文、说教感、情感博主腔、油腻霸道感，也禁止“你开心就行”“算了不反驳了”这类去角色化后谁都能说的安全模板句。',

    '### 输出前自检',
    '1. 这段话是不是只在表达 char 现在的态度，而没有讲任何故事？',
    '2. 这段话删掉角色名之后，是否像很多恋爱角色都能说？如果是，说明太泛，重写。',
    '3. 里面有没有任何输入未提供的事件、场景、动作、物件、时间落点？如果有，重写。',
    '4. 去掉所有画面感和过程感之后，这段回复还能成立吗？如果不能，说明它在靠编或靠模板支撑，重写。',
