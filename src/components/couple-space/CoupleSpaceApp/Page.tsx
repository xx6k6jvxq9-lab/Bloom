    ...
    <div className="absolute inset-0 bg-zinc-50 z-50 overflow-hidden flex flex-col">
      ...
      <div className={`relative z-10 flex-1 flex flex-col ${activeView === 'main' ? 'overflow-y-auto' : activeView === 'loveletter-detail' ? 'overflow-hidden' : 'couple-space-subview pt-[112px] overflow-hidden'}`}>
        <AnimatePresence mode="wait">
          ...
          {activeView === 'settings' && (
            <motion.div key="settings" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-4 space-y-4 flex-1 min-h-0 overflow-y-auto pb-24 no-scrollbar">
...
请你像在这条互记下面顺手回复我一样，接一句回应，简短自然，20字以内。`;
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-4 flex-1 min-h-0 flex flex-col w-full overflow-y-auto pb-24 no-scrollbar">
...
function LoveLettersView({ coupleSpace, updateSpace, updateSpaceForPartner, recordSettlement, user, partner, settings, chatHistory, masks = [], worldBooks = [], onOpenLetter }: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-4 flex-1 min-h-0 flex flex-col w-full relative overflow-y-auto pb-24 no-scrollbar">
...
function PostFeedView({ coupleSpace, updateSpace, updateSpaceForPartner, recordSettlement, user, partner, settings, onBack, chatHistory, masks, worldBooks }: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="px-4 flex-1 min-h-0 flex flex-col w-full overflow-y-auto pb-24 no-scrollbar">
