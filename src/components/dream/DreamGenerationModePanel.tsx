      ...
      {dreamGenerationModeOptions.map((option) => {
        ...
        return (
          <button
            ...
            style={{
              borderColor: active ? 'rgba(196,169,106,.48)' : 'rgba(196,169,106,.2)',
              backgroundColor: active ? 'rgba(196,169,106,.1)' : 'rgba(13,18,32,.55)',
              color: 'var(--gold)',
              boxShadow: active ? '0 0 22px rgba(196,169,106,.08)' : 'none',
            }}
          >
            <span
              ...
              style={{ backgroundColor: 'rgba(196,169,106,.08)' }}
            />
            <span className="pointer-events-none absolute inset-[3px] border" style={{ borderColor: active ? 'rgba(196,169,106,.38)' : 'rgba(196,169,106,.12)' }} />
            <div className="relative">
              <div className="flex items-center justify-between gap-4">
                ...
                <span
                  ...
                  style={{
                    borderColor: active ? 'rgba(196,169,106,.42)' : 'rgba(196,169,106,.2)',
                    backgroundColor: active ? 'rgba(196,169,106,.12)' : 'transparent',
                  }}
                >
                  {active ? '已选' : '可选'}
                </span>
  ...
  return (
    ...
    >
      <span
        ...
        style={{
          borderColor: checked ? 'rgba(196,169,106,.42)' : 'rgba(196,169,106,.22)',
          backgroundColor: checked ? 'rgba(196,169,106,.12)' : 'transparent',
        }}
      >
        {checked ? '✓' : ''}
      </span>
...
export function DreamGenerationModeTriggerButton({
  ...
  return (
    <button
      ...
      style={{
        borderColor: 'rgba(196,169,106,.28)',
        background: 'radial-gradient(circle, rgba(196,169,106,.14) 0%, rgba(196,169,106,.04) 58%, transparent 100%)',
        boxShadow: mode ? '0 0 20px rgba(196,169,106,.1)' : 'none',
        color: 'var(--gold)',
      }}
    >
      ...
      {mode ? (
        <span
          ...
          style={{
            ...
            boxShadow: '0 0 12px rgba(196,169,106,.44)',
          }}
        />
      ) : null}
    </button>
        ...
        <div className="mt-6 space-y-3">
          {selectedDefaultMode ? (
            <button
              ...
              style={{
                borderColor: 'rgba(196,169,106,.22)',
                backgroundColor: 'rgba(13,18,32,.55)',
              }}
            >
              清除默认模式
            </button>
