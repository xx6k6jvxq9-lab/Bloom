      ...
      {activeSection === 'main' && (
          ...
          >
            ...
            <div className="px-4 mt-6 space-y-4">
              ...
              <div className="rounded-3xl p-1 shadow-sm border border-zinc-100 divide-y divide-zinc-50/50 backdrop-blur-sm" style={bgStyle}>
                ...
                />
                <MenuButton
                  icon={<Compass className="text-zinc-900" size={20} />}
                  label="关系修正"
                  subLabel="默认自动推动，可进行调整"
        ...
        {activeSection === 'relationships' && (
          <RelationshipGraphPage
            ...
            globalBackground={globalBackground}
            onBack={() => setActiveSection('main')}
            onUpdateCharacters={setAppData ? (nextCharacters) => {
              setAppData((prev: any) => ({
                ...prev,
...
function DateRecordsDetailPage({
  savedDates,
  characters,
  onBack,
  globalBackground,
        ...
        {records.map(date => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[13px] text-zinc-500 italic">
                {date.location ? <span>地点：{date.location}</span> : null}
                {date.scenario ? <span>场景：{date.scenario}</span> : null}
              </div>
              <p className="mt-2 line-clamp-3 text-[14px] leading-relaxed text-zinc-700">
                {previewText}
              </p>
            </button>
          );
          ...
          >
            <div className="flex items-center gap-3 border-b border-zinc-100 px-4 pt-12 pb-4">
              ...
              <div>
                ...
                <p className="mt-0.5 text-[12px] text-zinc-500">
                  {characters?.find(c => c.id === selectedRecord.characterId)?.name || '未知角色'}
                </p>
              </div>
            </div>
    ...
    <div className={`absolute inset-0 z-[100] flex flex-col ${globalBackground ? 'bg-transparent' : 'bg-zinc-50'}`}>
      ...
      >
        ...
        <div className="flex-1">
          <h3 className="text-[17px] font-bold">约会记录</h3>
          <p className="mt-0.5 text-[12px] text-zinc-500">这里会保存你主动结束并归档的约会。</p>
        </div>
        <button
          type="button"
