...
function getEventTitle(request: FriendRequest, displayName: string) {
  switch (request.eventKind) {
    case 'user_unblocked_character':
      return '你解除了拉黑';
    case 'character_warned_user_from_chat':
      return `${displayName} 在聊天里划了边界`;
    case 'character_blocked_user_from_chat':
      return `${displayName} 在聊天里把你拉黑了`;
...
function getRoundStatusLabel(status: 'active' | 'resolved' | 'abandoned') {
  if (status === 'resolved') {
    return '本轮已完成';
  }
...
function getRoundStatusClass(status: 'active' | 'resolved' | 'abandoned') {
  if (status === 'resolved') {
    return 'bg-emerald-50 text-emerald-600';
  }
...
export function RelationshipThreadPage({
  ...
  const [composerMode, setComposerMode] = useState<'request' | 'reject' | null>(null);
  const [composerTargetRequestId, setComposerTargetRequestId] = useState<string | null>(null);

  const currentRoundId = parseRelationshipRoundPageKey(pageKey);
  const threadRequests = useMemo(() => (
    requests
      .filter((request) => isFriendRequestReleased(request))
      .filter((request) => matchFriendRequestToPageKey(request, pageKey))
      .sort((left, right) => {
        const leftRoundNo = getFriendRequestRelationshipRoundNo(left);
        const rightRoundNo = getFriendRequestRelationshipRoundNo(right);
  ...
  const relationshipRounds = useMemo(() => (
    characterId ? getCharacterRelationshipRounds(requests, characterId) : []
  ), [requests, characterId]);
  ...
  const latestStatusLabel = latestRequest?.isRelationshipEvent
    ...
    : latestRequest
      ...
      : '';
  const summaryStatusLabel = pendingIncomingRequest ? '待处理' : latestStatusLabel;
  ...
  const summaryLine = pendingIncomingRequest
    ? '当前有一条新的申请等待你处理'
    : currentRound
      ? `当前查看第 ${currentRound.roundNo} 轮 · ${getRoundStatusLabel(currentRound.status)} · 最近更新 ${formatRequestTime(latestRequest?.timestamp || Date.now())}`
  ...
  const openRequestComposer = () => {
    ...
    setComposerTargetRequestId(null);
  };

  const openRejectComposer = (requestId: string) => {
    setDraftMessage('');
    setComposerMode('reject');
    setComposerTargetRequestId(requestId);
  };
  ...
  const renderRequestEntry = (request: FriendRequest, index: number) => {
      ...
      >
        <div className="flex gap-3">
          <div className="flex w-4 shrink-0 flex-col items-center">
            ...
            {index < visibleRequests.length - 1 && <span className="mt-2 w-px flex-1 bg-zinc-100" />}
          </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 space-y-2.5">
              ...
              {!!request.userDecisionNote && (
                <ThreadTextBlock
                  label="你的回复"
                  text={request.userDecisionNote}
                />
              )}
            ...
            {isPending && isIncoming && !isEventRecord && (
              <div className="mt-3 flex gap-2">
                <button
                  type="button"
                  onClick={() => openRejectComposer(request.id)}
    ...
    <div ref={containerRef} className="absolute inset-0 z-[60] flex flex-col bg-zinc-50">
      <div className="min-h-[64px] border-b border-zinc-100 bg-white px-4 pb-3 pt-12 flex items-center gap-2">
        ...
        {onDeletePage ? (
          <button
            type="button"
            onClick={() => onDeletePage(pageKey)}
            className="rounded-full p-2 text-zinc-400 active:bg-zinc-100 active:text-zinc-600"
            aria-label="删除这条关系记录"
          >
            <Trash2 size={18} />
          </button>
      ...
      >
          ...
          {currentRound ? (
            <div className="overflow-hidden rounded-[28px] border border-zinc-100 bg-white shadow-sm">
              <div className="flex items-center justify-between gap-3 border-b border-zinc-100 px-4 py-3.5">
                <div className="min-w-0">
                  <div className="text-[14px] font-semibold text-zinc-900">第 {currentRound.roundNo} 轮关系修复</div>
                ...
                <div className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-medium ${getRoundStatusClass(currentRound.status)}`}>
                  {getRoundStatusLabel(currentRound.status)}
                </div>
              </div>

              {visibleRequests.map(renderRequestEntry)}
            </div>
          ) : (
            <div className="overflow-hidden rounded-[28px] border border-zinc-100 bg-white shadow-sm">
              {visibleRequests.map(renderRequestEntry)}
        ...
        {!pendingIncomingRequest && (
          <div className="mt-3 px-1 text-[12px] leading-5 text-zinc-400">
            这里会保留同一个角色每一轮关系修复的申请历史和关系动作记录，主聊天只保留少量必要提示。
          </div>
        )}
        ...
        {!!pendingIncomingRequest && composerMode === 'reject' && !!composerTargetRequestId && (
          <div className="mt-4">
            <div className="rounded-[26px] border border-zinc-100 bg-white p-4 shadow-sm">
              <div className="text-[14px] font-semibold text-zinc-900">拒绝前回一句</div>
              <div className="mt-1 text-[12px] text-zinc-400">这句会一起发给对方，影响对方接下来的反应或下一轮邀请。</div>
              <textarea
                value={draftMessage}
                onChange={(event) => setDraftMessage(event.target.value.slice(0, 120))}
              ...
              <div className="mt-4 flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => {
                    onReject(composerTargetRequestId);
                    closeComposer();
                  }}
                ...
                <button
                  type="button"
                  onClick={() => {
                    onReject(composerTargetRequestId, draftMessage.trim());
                    closeComposer();
                  }}
                  disabled={!draftMessage.trim()}
                  className={`flex-1 rounded-2xl border py-3 text-[14px] font-semibold ${
