...
基于当前蓝图目标、现有主链代码、最近一轮全盘体检，以及 2026-04 这一轮持续收口后的代码现实，当前阶段的优先级应明确更新为：

- `Phase B`：聊天 / 群聊 / 约会 / couple-space 主链已经基本接上，重点从“做出来”转向“收规则、收边界、收一致性、收手感”。
- `Phase B2`：旧路径清理仍在正式进行中，不应被视为附属提醒。兼容层应继续退到迁移职责，而不是长期参与主逻辑。
- `Phase C`：dating 与 couple-space 已并入统一关系链，已经具备闭环能力，但仍需继续收稳 write-back contract、结束闭环和跨场景连续性。
- `Phase D`：尚未开始完整建设，目前只有最小双层、summary、library 与 projection 雏形，距离正式 memory layer 还差明显一段。
- `Phase E`：时间感知与 initiative 已有局部产品化落点，但还没有形成真正通用、稳定的“角色继续生活”底座。

当前主线收益最高的工作，已经不是继续横向开很多新功能，而是优先把下面四层基础设施做稳：

1. 继续收稳角色边界与记忆语义，避免旧路径和旧字段重新回流主链。
2. 把私聊里的“角色重新上线 / 角色继续生活 / 角色有自己的在线状态”做深，避免角色像被冻结在上一条消息里。
3. 对群聊做一轮恢复后的体验校准，把“恢复了但操作感和以前不一样”单独作为主线收口，而不是混进群生态扩展。
4. 继续稳住 dating / couple-space 的 write-back、结束闭环和跨场景关系连续性，但不再把它们视为当前第一阻塞项。

在以上几层收稳之前，群生态不应正式展开。当前更合理的顺序是：

1. 角色边界别串
2. 记忆语义别脏
3. 私聊角色要像在继续生活
4. 群聊公共场合手感要校顺
5. 再进入 `P2-5.1 群生态关系感知底座`

换句话说，当前阶段最重要的任务，不是继续扩很多页面，而是先把“关系连续性底座”和“角色在线存在感”做成真正能承重的平台基础设施。只有在这层站稳之后，群生态、正式 memory layer 和更大范围的自主化能力才值得继续往前推。
...
- 聊天气泡样式仍有一条未收口 bug：单聊设置页写入的角色专属 CSS 气泡，在群聊里尚不能稳定命中真实消息气泡，当前表现仍可能退回原生白气泡
...
第一阶段已经收口。结合当前主链代码、最近一轮全盘体检以及蓝图实施节奏，当前状态应更明确概括为：

- 全项目当前更准确处于 `Phase B 中段后半`
- 旧路径清理已经进入 `Phase B2` 的正式收口阶段，而不是附属提醒
- 群聊子线当前更准确处于 `P2-5 群聊最小闭环中后段`
- 群聊当前已经不是“恢复能不能用”的阶段，而是进入“体验收口、活人感补强、旧路径清理”的阶段
- dating 与 couple-space 已进入统一关系链，但仍需继续收稳 write-back contract 与角色连续性表达
- `memorySummary` 的运行时主链兜底已经明显退位，旧记忆路径清理继续向前推进

从工程完成度看，当前可以大致判断为：

- 全项目总进度约 `76% - 82%`
- `Phase A` 底座约 `90%+`
- `Phase B` 主链与最小闭环约 `82% - 88%`
- `Phase B2` 旧路径清理约 `72% - 78%`
- `Phase C` 约会 / couple-space 接统一关系链约 `78% - 85%`
- `Phase D` 正式 memory layer 约 `35% - 45%`
- `Phase E` 时间感知 + initiative 底座约 `40% - 50%`

当前已经比较扎实的部分：

- 私聊、群聊、约会、情侣空间都已经接在统一 scene input / relationship projection 骨架上；
- 约会已经形成“开始 - 过程 - 结束 - 归档 - 记录回看”的最小闭环；
- couple-space 已有 execution boundary / commit route / draft / confirmation 的产品化链路；
- 群聊主链已经恢复，开始进入体验和边界收口阶段；
- 旧记忆路径退位已明显推进，运行时主链已不再直接把 `memorySummary` 当长期兜底。

当前仍然没有真正完成的关键层：

- 私聊角色“个人意识 / 自己在过生活”只做到了第一刀，虽然已经能识别 `continuous_scene / same_day_resume / resume_after_gap`，但还没有真正形成稳定的“角色在线存在感”；
- 群聊恢复后的手感和公共场合节奏还没专门校，当前依旧存在“恢复了但操作感不一样”的真实体感差异；
- 群聊相关 prompt / scene input 仍有历史乱码和脏块，影响后续调参与维护质量；
- 正式 memory layer 尚未开始完整建设，目前还只是最小双层与投影雏形；
- 群生态功能仍未正式展开，当前不应插队进入群投票、群接龙等扩展层。

因此，当前阶段最重要的结论，不是“还能再接多少功能”，而是：

**先把角色边界、记忆语义、角色在线存在感和群聊手感收稳，再为后续群生态扩展建立真正可依赖的关系底座。**
...
6. 当前阶段新增 UI 的选中态默认使用浅灰底，不使用纯黑底高亮；除非后续有明确品牌化方案，否则群资料、群世界书及后续同批新 UI 都遵守这一规则
